...
first_df = pd.DataFrame(directory_list, columns=['id'])
first_df['ms'] = 0
first_df['source'] = 'null'

all_df = first_df.copy()
for img_idx in tqdm(all_df.id.values):
    msk = cv2.imread(os.path.join(f'/home/hubmap-{CFG.data}x{CFG.data}/masks', img_idx), cv2.IMREAD_GRAYSCALE)
    if img_idx.split('_')[-1] == 'ext.png':
        all_df.loc[all_df['id'] == img_idx, 'source'] = 'ext'
    elif img_idx.split('_')[-1] == 'test.png':
        all_df.loc[all_df['id'] == img_idx, 'source'] = 'test'
    else:
        all_df.loc[all_df['id'] == img_idx, 'source'] = 'train'
    ...
    if np.sum(msk) == 0:
        ...
        msk_cls = 5
    all_df.loc[all_df['id'] == img_idx, 'ms'] = msk_cls


train_df = all_df.loc[all_df.source == 'train']
ext_df = all_df.loc[all_df.source == 'ext']
test_df = all_df.loc[all_df.source == 'test']

kfold = StratifiedKFold(n_splits=CFG.n_fold, shuffle=True, random_state=CFG.seed)
folds = [(idxT, idxV) for i, (idxT, idxV) in enumerate(kfold.split(train_df.id.values, train_df.ms.values))]
folds_ext = [np.concatenate((idxT, idxV)) for i, (idxT, idxV) in enumerate(kfold.split(ext_df.id.values, ext_df.ms.values))]
folds_test = [np.concatenate((idxT, idxV)) for i, (idxT, idxV) in enumerate(kfold.split(test_df.id.values, test_df.ms.values))]

for i in range(CFG.n_fold):
    (idxT, idxV) = folds[i]
    folds[i] = (np.concatenate((train_df.iloc[idxT].index, 
                                ext_df.iloc[folds_ext[i]].index, 
                                test_df.iloc[folds_test[i]].index)), 
                                train_df.iloc[idxV].index)

del folds_ext, folds_test
...
def prepare_train_valid_dataloader(df, trn_idx, val_idx):
    train_ = df.loc[trn_idx, :].reset_index(drop=True)
    valid_ = df.loc[val_idx, :].reset_index(drop=True)
    
    train_ds = HuBMAPDataset(train_, train='train', augment='base', transform=True)
    val_ds = HuBMAPDataset(valid_, train='val', augment='base', transform=True)
...
for fold, (train_idx, test_ids) in enumerate(folds):
    if fold in fold_train:
        print("Let's start training for {}-{}".format(CFG.encoder, CFG.base_model))
        
        print("fold:  {}    ----------------------------------------".format(fold))
        best = 0
        trainloader, validloader = prepare_train_valid_dataloader(all_df, train_idx, test_ids)

        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

        path = f'/home/champ_pkl/hubmap/result/{CFG.data}/{CFG.encoder}-{CFG.base_model}-{CFG.criterion}-{CFG.data}-FOLD-{fold}-model.pth'
        ...
        for epoch in range(CFG.epoch):
            ...
            if best_dice > best:
                best = best_dice
                torch.save(model.state_dict(), f'/home/champ_pkl/hubmap/result/{CFG.data}/{CFG.encoder}-{CFG.base_model}-{CFG.criterion}-{CFG.data}-FOLD-{fold}-model.pth')
